...
public class Employee {
   ...
   /* public void AddingNewAccountToAccountArray(String username , Client [] clients)
    ...
    void EmployeeCreatingAccount(Client [] clients  , int ID)
    {
        ...
        for (int i = 0; i < clients.length; i++)
        {
            if (clients[i].ID == ID)
            {
                ...
                // the program asking the employee to enter the username and password of the client to proccedd


                System.out.println("Enter 1 for saving account or 2 for current account or 0 to cancel:");

                // Write an fi condition w 2et3amel ba2a

                clients[i].MyAccounts[3] = new SavingAccount("blabla"  , 100000000);
